...
有一个大佬之前可能也遇到了类似的问题，所以他加了一个TCP的ip过滤，在进程模式下，匹配的ip进行代理或者不代理，其实我想要的还不仅仅是代理和不代理，还有一个block选项，从而实现让客户都自己去更好请求的ip，而不是直接无响应。
...
一般来说是以掩码长度为准的，这里应该是Netch本身有bug，如果**你后续掩码位置填写的不是0，会导致这条规则根本不生效。**
...
主要通过 redirector/tcpConnectRequest 来处理整个TCP连接，可以在这里加个筛选来过滤IP，但是我感觉在这个位置处理已经有点晚了。



## Summary

未完待续，后续还有可能会改动一下Netch对于IP的过滤部分
