...
![image-20240803214320785](https://img.elmagnifico.tech/static/upload/elmagnifico/202408032143869.png)

色品图是怎么得到的，又意味着什么，他能怎么指导我们使用或者认识颜色？

#### 认识颜色

大部分是参考[Lele Feng](https://www.zhihu.com/people/lele-feng-15)的文章（有部分错误），但是对于已知的人来说，我需要更精简一些的推理和结果，详细的可以看原文章。
...
![image-20240803214632646](https://img.elmagnifico.tech/static/upload/elmagnifico/202408032146712.png)
...
相当于是规定了用RGB三种独立的色光去描述现实中我们能看到颜色光谱，这里有点小问题，就是这个负光的问题，负光就简单理解为从相反方向发射过来的一簇光，用来抵消当前的光，从而他就是负光了。

- 负光主要是因为通过实验发现，自然界中的某些光是无法用RGB三色进行合成的（某种颜色光强已经到达0了），这个时候对自然光中加入一定的RGB反而可以让二者相等，而加入的这个部分就变成了负值，一般红色部分比较多一些。
- 这也导致了这个色彩空间的坐标系并不是正交，而是有一定角度的坐标系
...
![image-20240803213309738](https://img.elmagnifico.tech/static/upload/elmagnifico/202408032133795.png)

前面的RGB组成的空间中有负值，这样的光实际中存在，但是要通过简单的RGB发出来就不行了，

同时由于这个坐标轴是非正交的，计算起来就很麻烦，所以为了解决这个问题，消除负值，CIE 1931 XYZ color space 中就重新规定了xyz轴所对应的发出的光，让颜色三轴变成了正交的，但是这样还不够，这里会发现显示同一个颜色，他在空间中是立体的，统一颜色所在的直线上，不同的点表示的是亮度不同。

为了让我们更清晰明确的选择颜色或者看到颜色的范围，进行了xyz的归一化处理（x+y+z=1），那么在归一化以后得到的这个颜色平面，就是等亮度颜色空间，这样就能很方便计算颜色和亮度了。

![image-20240803214106387](https://img.elmagnifico.tech/static/upload/elmagnifico/202408032141438.png)

色度图中光谱轨迹线所围成的区域代表了通过混合各种不同波长单色光我们能感知到的颜色，这算是第一次统一了颜色标准。

但是也有缺点，CIE 1931 xy色度图为了简化（去掉了z轴，将其投影到了xy屏幕上），他将三维轴降到了2维，这样描述的颜色都是没有说明亮度（过亮，你会认为是白色，过暗你会认为是黑色，但是光本身是不会骗人的，只是你识别的灵敏度有限而已）
...
而常见的颜色空间，比如sRGB，AdobeRGB，NTSC，P3，rec601和rec709，BT.2020等等，他们相当于上面色度图所表示范围的一个子集，原因很简单大家谁都不能完整描绘整个色度图，而随着科技进步，描述的范围越来越大，相当于我们能看到的颜色越来越丰富了。

- 白点，由于这是二维图，所以白点就是用来描述那个第三维的，当然只是近似而已。白点的x，y相当于对应的RGB的1，1，1的值。而RGB是线性的所以相当于用了归一化，认为大于某个值就是R=1。实际中白点所代表的RGB三个单色光的强度可能是不同，合成以后的白点自然也就不同，但是他代表了实际中的最大亮度
...
到这里基本颜色空间中重要的东西都说了，还有一个伽马值，这东西游戏里经常有太黑看不清，调整一下伽马值就看的到了，其实这个伽马就是传递函数的一个参数而已，本身传递是非线性关系，gamma就是这个函数的幂指数，所以调整一点效果就非常大。调整伽马是让黑的地方变灰了，但是同时其他颜色可能也会呈现出过饱和或者变灰的情况。颜色空间实际上更复杂一些，还有一些其他参数这里没提到，他们与环境的关系比较大一些。

- 颜色空间转换，因为硬件非常多，有老的有新的，但是大家能支持显示的颜色范围也不同，为了在不同设备之间显示的时候尽量减少偏差，就需要对颜色进行转换，而不是直接将sRGB的颜色拿到BT.2020下使用，其实就是将其还原到xyz三刺激轴下，然后再转向BT.2020即可，经过这样的操作就叫颜色空间转换
...
> https://www.bilibili.com/video/BV1x34y1g7hh
...
>
> https://www.bilibili.com/video/BV1E2421c7NM
>
> https://www.bilibili.com/video/BV1x34y1g7hh
