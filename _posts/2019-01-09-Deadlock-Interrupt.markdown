...
在I2C出错以后,抛出了错误中断,而在错误中断中呢,竟然发现他在清标志位的时候清错了,该清的没清,进而出现了无限中断,而中断处理函数却在第一次处理的时候将应该的中断处理函数置为了NULL(虽然他就算不置NULL也清不掉)
...
为什么中断没有清掉这个标志位呢,下面的代码中Disable和Enable都是用的一样的操作,而实际上这个寄存器需要写1清,这里全都是或上1,然后最后的宏是取反后做与,就相当于保留了所有的标志位.
