...
description: "WIFI模块ARP刷新问题，二层、三层、交换机、AP。"
date:       2025-09-11
update:     2025-09-11
author:     "elmagnifico"
header-img: "img/head-2025-09-10.jpg"
catalog:    true
tobecontinued: false
tags:
    - Network
...
设备不上线的问题太多了，各种各样的，还都是偶发，没有现场，无法复现，针对这个问题造成的情景很多，下面一一列举：

1. 设备上了AP，但是不上PC，重启电脑就好了
2. 设备上了AP，但是不上PC，ping一下设备就好了
3. 设备不上线，重启设备可以好
4. 设备不上线，重启AP可以好
...
二层局域网中，如果设备之间通信不仅仅需要知道IP地址，还需要知道mac地址，ARP的作用就是在加入一个新的局域网的时候自动广播一次，告诉局域网内的其他设备，它加入了，其他设备就会存储它的mac和ip，下次通信时就知道包要发给哪个mac地址了。

ARP比较特殊，它是网络层协议，但是工作在数据链路层
...
三层设备，路由器，这种设备会在内部维护路由表，联动ARP的mac表，它会自主进行维护，所以它会主动发起ARP广播

- 以上只是通常意义下的情况，特殊情况二层也是会广播的，比如在和二层设备通信时，它肯定需要主动广播ARP
...
泛指客户端设备，比如WIFI模块、PC，它们在与PC或者其他设备通信时，内部会维护一个mac和ip的地址表，正常情况下这个表会在加入新网络或者网络断开时重新刷新，或者快速老化
...
我们目前出现不上线的问题，基本都在这里，设备端切换SSID后，没有更新它的目标mac地址（PC的mac地址），它发送的地址一直都是错误的，这个mac地址不存在，直接就被丢弃了，所以连上了AP，但是PC收不到。要等到设备主动更新它的mac地址、PC重启、网口重启、网口重设IP、电脑重启等任一条件发生，这个设备才会正常上线。
...
4. 2个AP分别连2个电脑，各开一个SSID，切换，ping一下设备，没有问题
...
neighborcachelimit 是新的取代ARP的，需要独立设置表项大小，PC上ARP的机制或者叫法改来改去的，和系统有很大关系
