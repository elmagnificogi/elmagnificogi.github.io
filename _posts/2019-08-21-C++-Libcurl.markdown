...
![SMMS](https://i.loli.net/2019/08/21/DSvGs7OjxmEF5Yo.png)
...
![SMMS](https://i.loli.net/2019/08/21/G6vYdJhSNCXcFfP.png)
...
![SMMS](https://i.loli.net/2019/08/21/zuhJSvnktBaC2sW.png)
...
我需要的一个12版本的VS（我本机上还有一个12版本的VS）代码，同时我需要的是一个静态库所以 mode是static，机器是64位，不需要debug，然后回车进行编译。

![SMMS](https://i.loli.net/2019/08/21/PN3G7Dsl8ajt4Uo.png)
...
1.引入头文件目录，引入库文件目录，要注意的是这里一定是要引入include还有lib，否则很有可能内部识别路径出错
  ...
  ![1566382204007](C:\Users\elmagnifico\AppData\Roaming\Typora\typora-user-images\1566382204007.png)

2.预处理器中增加 CURL_STATICLIB
    ![SMMS](https://i.loli.net/2019/08/21/fh2pLrCeclit7J9.png)
...
3.链接器的输入中增加以下几个lib
  ...
  ![SMMS](https://i.loli.net/2019/08/21/I1bz2MBPnd4rCcs.png)

4.c++代码生成中运行库选择/MD模式
    ![SMMS](https://i.loli.net/2019/08/21/oiOhgkFnur6tLsy.png)
...
- /MD 这种模式下，在生成代码时使用的是静态链接库，也就是直接将所使用的非我所写的code的定义直接拿过来用，而不是去依赖其他文件，讲道理前面对libcurl选择了一堆静态编译，实际上使用的时候却用了动态，这就很维和
...
![SMMS](https://i.loli.net/2019/08/21/iZXITF8BW7zaGfP.png)
...
```
nmake /f Makefile.vc mode=static VC=12 ENABLE_IDN=no ENABLE_SSPI=no DEBUG=no MACHINE=x64 RTLIBCFG=static
```
